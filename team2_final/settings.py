...
# 1차: ALLOWED_HOSTS 기반으로 CSRF_ORIGINS 채우기
for _h in ALLOWED_HOSTS:
    if _h:
        _add_csrf_origins(_h)

# 2차: 도커/로컬에서 자주 쓰는 내부 호스트들 강제 보정
_EXTRA_HOSTS = [
    "nginx",
    "api",
    "team2_nginx",
...
def _sslmode() -> str:
    ...
    return env_get(
        "DB_SSLMODE",
        env_get("POSTGRES_SSLMODE", env_get("SSLMODE", "require")),
    )
...
# ─────────────────────────────────────────────────────────────────────────────
# 6.x) Redis 캐시 설정
# - docker-compose.local.yml 에서 REDIS_URL 이 주입되면 Redis 사용
# - 없으면 로컬 메모리 캐시(LocMem) 사용
# ─────────────────────────────────────────────────────────────────────────────
...
if REDIS_URL:
    ...
    # AI/이미지 분석 같은 무거운 연산 캐시 시간 (기본 1시간)
    AI_MEAL_CACHE_TIMEOUT = int(env_get("AI_MEAL_CACHE_TIMEOUT", str(60 * 60)))
else:
    CACHES = {
        "default": {
    ...
    # Redis를 안 쓰는 환경에서는 캐시 비활성화 느낌으로 0
    AI_MEAL_CACHE_TIMEOUT = 0
